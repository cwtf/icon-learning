...
The remaining four categories (Microsoft & Digital Skills; Finance & Taxation; Supply Chain & Shipping; Communication & Personal Effectiveness) are reachable via the `/programs` catalog. A `Browse all 10 categories →` CTA closes the section.
...
Two view modes: by **category** (default, grouped accordions of the ten categories from §7.1) and **flat list** (alphabetical). Search collapses both into a single filtered result list.
...
7. **"Meet the trainer"** *(optional — renders only when the course JSON sets `trainerId`)* — a single card: name, role, a short verified bio (~60–90 words, drawn from the trainer's own CV, no unverified credentials), 2–3 credential highlight pills, and a "proof of work" link list to live, public projects. Trainer records live in `src/content/trainers.ts` (single source, referenced by id — never duplicate bios into course JSON). When present, the Course JSON-LD gains an `instructor` Person with `sameAs` profile links. No photo until an approved headshot exists. Currently used by the AI category courses only; do not invent profiles for courses whose trainer is unconfirmed.
8. **"Bring it to your team"** — closer card. Heading + one-sentence nudge (*"Tell us your team size and dates — we'll send the outline and a quote."*), `Get the full outline` primary, `WhatsApp us` secondary.
...
Ten top-level categories, derived from the actual catalog in `course/documents/`:
